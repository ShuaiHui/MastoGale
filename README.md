...
- 📈 **热门嘟文（Trends）**：实时抓取并浏览实例全网最热门的流行嘟文，支持无限滚动与全部交互
...
### v1.2.0（热门嘟文与界面优化版）
- **全新“热门嘟文”（Trending Statuses）选项卡**：将原闲置的“私信”选项卡重构为实时热门嘟文面板，支持抓取长毛象 `/api/v1/trends/statuses` 全网流行嘟文，向下滚动自动触发 offset 无限增量分页。
- **全交互能力融合**：热门嘟文完美打通主时间线底层 DOM，支持点赞（Fav）、转推（Boost）、回复、上下文查看以及图片灯箱缩放。
- **退出按钮对齐修复**：在选项卡界面中对账号管理按钮应用 `!important` 样式保护，解决由于全局 CSS 重置引起的按钮上下偏移，使退出按钮与活跃徽章中线完美对齐。
- **净化冗余选项**：彻底移除长毛象中不适用的“生日提醒”和“私信”通知开关，精简并润色使用技巧中的历史饭否遗留术语。
- **修复关于页面发送统计**：重写 AJAX 成功监听器以适配 Mastodon API 发帖及媒体上传路径，恢复“关于”页面中发送消息与照片条数的实时自动累加记录。
