...
var r = PREFiX.user || (PREFiX.accessToken ? Ripple(PREFiX.accessToken) : null);
if (r && !PREFiX.user) {
	PREFiX.user = r;
}
...
$(function() {
	require('avalon.live', function() {
		...
		setTimeout(function() {
			...
			var current = getCurrent();
			if (current && typeof current.initialize === 'function') {
				current.initialize();
			}
...
onunload = function() {
	...
	if ($main && $main[0] && $main[0].scrollTop < 30)
		cutStream();
	if (is_panel_mode) {
		var pos = {
			x: screenX,
